...
### TestFixtures (Required for All Test Data)

Before using builders, check if `TestFixtures` already covers your needs. All test credentials (passwords, tokens, API keys) MUST go through TestFixtures — never inline.

```swift
// GOOD — use TestFixtures
sut.password = TestFixtures.validPassword()
let item = TestFixtures.item(title: "Custom")

// BAD — hardcoded inline
sut.password = "MyPassword123"
```

Use builders only for complex objects with many optional properties. Simple test data belongs in `TestFixtures`.
